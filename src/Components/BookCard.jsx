...
const BookCard = ({ book }) => {
  return (
    <div className="p-2 border-2 border-gray-500 w-64 shadow-sm hover:scale-105 transition-all cursor-pointer h-fit" key={book.id} >
      ...
      <Link to={`/book/${book.id}`}><button className='font-Poppins px-2 py-1.5 bg-blue-800 text-white text-xs float-right'>View details</button></Link>
    </div>
  )
}
